...
def get_cod_freq(cds: str, ID: str) -> pd.DataFrame:
    ...
    df_codcnt['Amino_acid'] = [codon_aa[codon]
                               for codon in df_codcnt['Codon'].values]
...
def compute_rscu_weights(df_codcnt: pd.DataFrame) -> pd.DataFrame:
    ...
    aa = df_codcnt['Amino_acid'].unique()
    df_list = []
    for a in aa:
        ...
        df_list.append(d)
        # some genomes may not use any amino acids
        rscu = pd.concat(df_list).fillna(0)
    return rscu
...
def main():
    ...
    parser.add_argument('-CDS', help='Path to fasta file with  coding sequences',
                        type=str, required=True, metavar='')
    parser.add_argument('-out', help='Path of destination folder for output file',
                        type=str, default='./file_out.rscu', metavar='')
    args = parser.parse_args()

    # preprocess fasta to a paired list of headers and sequences
    ...
    for rscu in df_list:  # rname rscu_list
        ...
        r3 = r3[cols[-2:]+cols[:-2]]
        comb.append(r3)
    pd.concat(comb, axis=0).reset_index(drop=True).to_csv(
        args.out+'_rscu.csv', index=False)
