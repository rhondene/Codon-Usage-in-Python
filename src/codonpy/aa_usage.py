...
def get_cod_freq(headers: list, seqs: list) -> pd.DataFrame:
    ...
    for i, cds in enumerate(seqs):
        if len(cds) % 3 != 0:
            ...
            print(
                f"WARNING: Skipping {ID} Length of CDS is not a multiple of 3.")
            continue
    ...
    df_codcount['Amino_Acid'] = [codon_to_aa[codon]
                                 # add amino acid column
                                 for codon in df_codcount['Codon'].values]
...
def compute_aa_usage(species: str, df_rscu: pd.DataFrame) -> pd.DataFrame:
    ...
    base_freq = {'U': 0.220, "A": 0.303, 'C': 0.217,
                 'G': 0.261}  # Base frequency in nature
    ...
    for amino, group in df_rscu.groupby('Amino_Acid'):
        ...
        for codon in group['Codon'].values:
            expected_aa_usage += base_freq[codon[0]] * \
                base_freq[codon[1]]*base_freq[codon[2]]
        expected_aa_usage = expected_aa_usage*1.057  # correction factor
        obs_freq = group['Obs_Freq'].sum() / df_rscu['Obs_Freq'].sum()
        aa_usage[amino] = [expected_aa_usage*100, obs_freq * 100,
                           group['Obs_Freq'].sum(), len(group['Codon'].unique())]

    aa_df = pd.DataFrame.from_dict(aa_usage, orient='index', columns=[
                                   'Expected_Freq(%)', 'Obs_Freq(%)', 'Abs_Freq', 'Num_Codons']).reset_index()
    aa_df.columns = [
        'Amino_acid', 'Expected_Freq(%)', 'Obs_Freq(%)', 'Abs_Freq', 'Num_Codons']
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Computes Observed and Expected Amino Acid Frequencies + RSCU from FASTA of Coding Sequences according to https://qubeshub.org/publications/979/serve/1/3067?el=1&download=1 .")
    parser.add_argument("-CDS", help="Path to the input FASTA file.",
                        type=str, required=True, metavar='')
    parser.add_argument("-out", help="Prefix for the output files.",
                        type=str, required=True, metavar='')
    parser.add_argument(
        "-keep_sixfold_aa", help=" True|False Whether to consolidate sixfold degenerate amino acids or break up into 4fold and 2fold. Default is False.", default=False, metavar='')
    args = parser.parse_args()

    headers, seqs = get_seqs(args.CDS)
    # computes absolute codon frequencies
    df_rscu = get_cod_freq(headers, seqs)
