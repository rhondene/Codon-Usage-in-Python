...
def get_cod_freq(headers: list, seqs: list):
    ...
    df_codcount['Amino_Acid'] = [codon_aa[codon]
...
def compute_rscu_weights(df_codcount: pd.DataFrame) -> pd.DataFrame:
    ...
    # make a list of all amino acids to iterate over
    aa = df_codcount['Amino_Acid'].unique()
    df_list = []
    for a in aa:
        d = aa_groups.get_group(a)
        d['RSCU'] = d['Obs_Freq'].values / \
            d['Obs_Freq'].mean()  # obs/expected freq
        d['Relative_Adaptive_Weights'] = d['RSCU'].values/d['RSCU'].max()
        d['optimal'] = [True if rscu == d['RSCU'].max(
        ) else False for rscu in d['RSCU'].values]  # marks optimal codon
        df_list.append(d)
...
def main():
    ...
    parser.add_argument('-CDS', help='Path to fasta file with species coding sequences',
                        type=str, required=True, metavar='')
    parser.add_argument('-out', help='Path of destination folder for output file (text file)',
                        type=str, default='./file_out.rscu', metavar='')

    args = parser.parse_args()

    # formats fasta into csv of sequences
    headers, seqs = fix_fasta.fix_fasta(args.CDS)
    # computes absolute codon frequencies
    df_codcount = get_cod_freq(headers, seqs)
    # computes RSCU and adaptive weights
    rscu = compute_rscu_weights(df_codcount)
