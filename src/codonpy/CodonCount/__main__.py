...
def get_cod_freq(geneID: str, cds: str) -> pd.DataFrame:
    ...
    codon_count = {codon: count / total_cod for codon,
                   count in codon_count.items()}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="This program takes a fasta file of coding sequences and outputs a csv table of the fractional codon content for each gene. Written by Rhondene Wint (rwint@ucmerced.edu).")
    parser.add_argument('-CDS', help='Name or Path of fasta file with  coding sequences',
                        type=str, required=True, metavar='')
    parser.add_argument(
        '-out', help='Name or Path of destination folder for output file', type=str, metavar='')
    ...
    for i in range(transcriptome.shape[0]):
        ...
        if len(cds) % 3 != 0:
            print(
                f"WARNING: Skipping {geneID}! Length of CDS is not a multiple of 3.")
            continue
